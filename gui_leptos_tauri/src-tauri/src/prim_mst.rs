...
pub fn prim_mst(edges: Vec<(OrderedFloat<f32>, Net, PointPair)>) -> Vec<(OrderedFloat<f32>, Net, PointPair)> {
    ...
    let point_to_index: HashMap<Point, i32> = index_to_point
        ...
        .collect();
    let edges: Vec<(OrderedFloat<f32>, Net, i32, i32)> = edges.into_iter()
        ...
        .collect();
    let undirected_graph: Vec<Vec<(OrderedFloat<f32>, i32)>> = {
    ...
    let mut mst_edges: Vec<(OrderedFloat<f32>, Net, i32, i32)> = vec![];
    let mut visited = std::collections::HashSet::new();
    // let edges = edges.into_iter()
    //     .map(|(weight, net, start, end)| (weight, Option<(Net, Point, Point)>::Some((net, start, end))))
    //     .collect::<Vec<_>>();
    let mut priority_queue: BinaryHeap<Reverse<(OrderedFloat<f32>, i32, i32)>> = BinaryHeap::new();
    ...
    //     .collect::<Vec<_>>();
    let mst_edges: Vec<(OrderedFloat<f32>, Net, PointPair)> = mst_edges.into_iter()
        .map(|(weight, net, start, end)| {
            let start_point = index_to_point[start as usize].clone();
            let end_point = index_to_point[end as usize].clone();
